...
///
/// Used internally during proximity queries (e.g., `tf::nearness_search`).
/// Coonstruct it using the helper function `tf::make_aabb_metrics`
///
...
/// @brief Compute distance metrics between two AABBs for use in dual-tree queries.
///
/// Returns an `aabb_metrics<RealT>` struct containing:
...
///
/// These values are used for guiding dual-tree traversal order and early pruning.
///
